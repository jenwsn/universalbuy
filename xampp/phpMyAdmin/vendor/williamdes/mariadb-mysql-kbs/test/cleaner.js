...
module.exports = function() {
    suite('cleaner', function() {
        test('clean cli html code', function(done) {
            const cli = cleaner.cleanCli('<code>--images-argument</code>');
            expect(cli).to.equal('--images-argument');
            done();
        });
        test('clean cli html code not closed', function(done) {
            const cli = cleaner.cleanCli('<code>--images-argument');
            expect(cli).to.equal('--images-argument');
            done();
        });
        test('clean cli nothing to clean', function(done) {
            const cli = cleaner.cleanCli('--images-argument');
            expect(cli).to.equal('--images-argument');
            done();
        });
