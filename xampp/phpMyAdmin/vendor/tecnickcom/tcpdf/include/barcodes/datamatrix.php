...
// -----------------------------------------------------------------------------
	...
	/**
	 * The look-ahead images scans the data to be encoded to find the best mode (Annex P - steps from J to S).
			...
			switch ($enc) {
				case ENC_ASCII: { // STEP B. While in ASCII encodation
					if (($data_length > 1) AND ($pos < ($data_length - 1)) AND ($this->isCharMode(ord($data[$pos]), ENC_ASCII_NUM) AND $this->isCharMode(ord($data[$pos + 1]), ENC_ASCII_NUM))) {
						...
						// 2. If the look-ahead images (starting at step J) indicates another mode, switch to that mode.
				...
				case ENC_X12 : { // ANSI X12
					...
					do {
						...
						if ($p >= 3) {
							...
							$cw_num += 2;
							$pos = $epos;
							// 1. If the C40 encoding is at the point of starting a new double symbol character and if the look-ahead images (starting at step J) indicates another mode, switch to that mode.
							$newenc = $this->lookAheadTest($data, $pos, $enc);
							if ($newenc != $enc) {
								// switch to new encoding
								$enc = $newenc;
				...
				case ENC_BASE256: { // G. While in Base 256 (B256) encodation
					...
					while (($pos < $data_length) AND ($field_length <= 1555)) {
						$newenc = $this->lookAheadTest($data, $pos, $enc);
						if ($newenc != $enc) {
							// 1. If the look-ahead images (starting at step J) indicates another mode, switch to that mode.
							$enc = $newenc;
