...
averylonglongfieldname X(1024) DEFAULT 'images',
...
";
	...
	if ($dbType == 'mysqlt') {
		$db->Connect('localhost', "root", "", "images");
		$dict->SetSchema('');
		$sqla2 = $dict->ChangeTableSQL('adoxyz',$flds);
		if ($sqla2) printsqla($dbType,$sqla2);
	}
	if ($dbType == 'postgres') {
		if (@$db->Connect('localhost', "tester", "images", "images"));
		$dict->SetSchema('');
		$sqla2 = $dict->ChangeTableSQL('adoxyz',$flds);
		if ($sqla2) printsqla($dbType,$sqla2);
	}
	...
	if ($dbType == 'odbc_mssql') {
		...
		if (@$db->Connect($dsn, "sa", "natsoft", "images"));
		$dict->SetSchema('');
		$sqla2 = $dict->ChangeTableSQL('adoxyz',$flds);
		if ($sqla2) printsqla($dbType,$sqla2);
	}
