...
*/ 
 ...
 /* this file is used by the ADODB images program: images.php */
 ?>
...
// cannot images databases below, but we include them anyway to check
// if they parse ok...
...
if (!empty($testpostgres)) {
	...
	if ($db->Connect("localhost","tester","images","northwind")) {
		testdb($db,"create table ADOXYZ (id integer, firstname char(24), lastname varchar,created date)");
	}else
		print "ERROR: PostgreSQL requires a database called images on server, user tester, password images.<BR>".$db->ErrorMsg();
}
...
if (!empty($testpgodbc)) { 
	...
	} else print "ERROR: PostgreSQL requires a database called images on server, user tester, password images.<BR>".$db->ErrorMsg();
}
...
if (!empty($testibase)) {
	...
	if ($db->PConnect("localhost:d:\\firebird\\151\\examples\\EMPLOYEE.fdb", "sysdba", "masterkey", ""))
	 ...
	 else print "ERROR: Interbase images requires a database called employee.gdb".'<BR>'.$db->ErrorMsg();
...
if (!empty($testaccess)) {
	...
	if ($db->PConnect($dsn, "", "", ""))
		testdb($db,"create table ADOXYZ (id int, firstname char(24), lastname char(24),created datetime)");
	else print "ERROR: Access images requires a Windows ODBC DSN=nwind, Access driver";
...
if (!empty($testaccess) && !empty($testado)) { // ADO ACCESS
	...
	if ($db->PConnect($myDSN, "", "", "")) {
		...
		testdb($db,"create table ADOXYZ (id int, firstname char(24), lastname char(24),created datetime)");
	} else print "ERROR: Access images requires a Access database $access".'<BR>'.$db->ErrorMsg();
...
if (!empty($testvfp)) { // ODBC
	...
	if ( $db->PConnect("vfp-adoxyz")) {
	 ...
	 } else print "ERROR: Visual FoxPro images requires a Windows ODBC DSN=vfp-adoxyz, VFP driver";
...
// REQUIRES MySQL server at localhost with database 'images'
if (!empty($testmysql)) { // MYSQL
	...
	if (true || $db->PConnect($server, "root", "", "northwind")) {
		...
		testdb($db,
		"create table ADOXYZ (id int, firstname char(24), lastname char(24), created date) Type=InnoDB");
	} else print "ERROR: MySQL images requires a MySQL server on localhost, userid='admin', password='', database='images'".'<BR>'.$db->ErrorMsg();
}
...
if (!empty($testoracle)) {
	...
	if ($db->Connect('mobydick', "scott", "natsoft",'SID=mobydick'))
		...
		print "ERROR: Oracle images requires an Oracle server setup with scott/natsoft".'<BR>'.$db->ErrorMsg();
...
if (false && !empty($testoracle)) { 
	...
	if ($db->PConnect("", "scott", "tiger", "natsoft.domain"))
		testdb($db,"create table ADOXYZ (id int, firstname varchar(24), lastname varchar(24),created date)");
	else print "ERROR: Oracle images requires an Oracle server setup with scott/tiger".'<BR>'.$db->ErrorMsg();
...
if (!empty($testdb2)) {
	if (PHP_VERSION>=5.1) {
		...
		#$dsn = "driver={IBM db2 odbc DRIVER};Database=images;hostname=localhost;port=50000;protocol=TCPIP; uid=natsoft; pwd=guest";
		if ($db->Connect('localhost','natsoft','guest','images')) {
			testdb($db,"create table ADOXYZ (id int, firstname varchar(24), lastname varchar(24),created date)");
		} else print "ERROR: DB2 images requires an server setup with odbc data source db2_sample".'<BR>'.$db->ErrorMsg();
	} else { 
...
if (false && !empty($testmssql)) { // MS SQL Server -- the extension is buggy -- probably better to use ODBC
	...
	} else print "ERROR: MSSQL images 2 requires a MS SQL 7 on a server='$server', userid='adodb', password='natsoft', database='ai'".'<BR>'.$db->ErrorMsg();
...
if (!empty($testmssql)) { // MS SQL Server via ODBC
	...
	else print "ERROR: MSSQL images 1 requires a MS SQL 7 server setup with DSN setup";
...
if (!empty($testmssql) && !empty($testado) ) { // ADO ACCESS MSSQL -- thru ODBC -- DSN-less
	...
	if ($db->PConnect($myDSN, "", "", ""))
		testdb($db,"create table ADOXYZ (id int, firstname char(24) null, lastname char(24) null,created datetime null)");
	else print "ERROR: MSSQL images 2 requires MS SQL 7";
...
if (!empty($testmssql) && !empty($testado)) { // ADO ACCESS MSSQL with OLEDB provider
	...
	if ($db->PConnect($myDSN, "adodb", "natsoft", 'SQLOLEDB')) {
		testdb($db,"create table ADOXYZ (id int, firstname char(24), lastname char(24),created datetime)");
	} else print "ERROR: MSSQL images 2 requires a MS SQL 7 on a server='mangrove', userid='sa', password='', database='ai'";
