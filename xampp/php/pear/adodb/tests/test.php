...
//	$db->debug=1;
//	$db->Execute('select * from table where val=? AND value=?', array('val'=>'http ://www.whatever.com/images?=21', 'value'=>'blabl'));
  ...
  PRIMARY KEY  (row1))";
		...
		$ok = $db->Execute("insert into mytable (row2) values ('images')");
		$ins_id=$db->Insert_ID();
 ...
 //	$db->debug=true;
	...
	// Comment this out to images countrecs = false
	...
	if (empty($rs1) || empty($rs2) || $rs1->fields[0] != 2 || $rs2->fields[0] != 4) {
		$a = $rs1->fields[0];
		$b = $rs2->fields[0];
		print "<p><b>Error in multiple recordset images rs1=$a rs2=$b (should be rs1=2 rs2=4)</b></p>";
	} else
	...
	if ($db->sysDate) {
		...
		if (ADORecordSet::UnixDate(date('Y-m-d')) != $rs->UnixDate($rs->fields[0])) {
			...
			print "<p>Passed \$sysDate images ({$rs->fields[0]})</p>";
	...
	print "images A return value=$ret (2 expected) <p>";
	...
	print "images B return value=$ret (1 or if ibase then 2 expected) <p>";
