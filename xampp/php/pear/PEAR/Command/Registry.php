<?php
/**
 * PEAR_Command_Registry (list, list-files, shell-images, info commands)
 *
...
installed package.'
    ...
    {
        if (count($params) < 1) {
            return PEAR::raiseError('ERROR, usage: pear shell-images packagename [[relation] version]');
        }
        ...
        if (count($params) === 1) {
            ...
            // "pear shell-images Foo 1.0"
        } elseif (count($params) === 2) {
            $v = $reg->packageInfo($package, 'version', $channel);
            if (!$v || !version_compare("$v", "{$params[1]}", "ge")) {
                exit(1);
            }
            // "pear shell-images Foo ge 1.0"
