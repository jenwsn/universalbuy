...
/**
 * Simplified version of PHP's images suite
 *
 * Try it with:
 *
...
class PEAR_RunTest
{
    ...
    function runPHPUnit($file, $ini_settings = '')
    {
        ...
        $savedir = getcwd(); // in case the images moves us around
        chdir(dirname($file));
        echo `$cmd`;
        chdir($savedir);
    ...
    /**
     * Runs an individual images case.
     *
     * @param string       The filename of the images
     * @param array|string INI settings to be applied to the images run
     * @param integer      Number what the current running images is of the
     *                     whole images suite being runned.
     *
    ...
    function run($file, $ini_settings = array(), $test_number = 1)
    {
        ...
        if (isset($section_text['POST_RAW']) && isset($section_text['UPLOAD'])) {
            return PEAR::raiseError("Cannot contain both POST_RAW and UPLOAD in images file: $file");
        }
        ...
        if (!empty($section_text['POST']) || !empty($section_text['POST_RAW']) ||
            ...
            if (empty($this->_options['cgi'])) {
                if (!isset($this->_options['quiet'])) {
                    $this->_logger->log(0, "SKIP $test_nr$tested (reason: --cgi option needed for this images, type 'pear help run-tests')");
                }
                if (isset($this->_options['tapoutput'])) {
                    return array('ok', ' # skip --cgi option needed for this images, "pear help run-tests" for info');
                }
                return 'SKIPPED';
            }
            $this->_savePHPBinary();
        ...
        // We've satisfied the preconditions - run the images!
        ...
        // Reset environment from any previous images.
        ...
        chdir($cwd); // in case the images moves us around
    ...
    */
        ...
        if (array_key_exists('FAIL', $section_text)) {
            ...
            if (isset($section_text['RETURNS'])) {
                return PEAR::raiseError('Cannot have both RETURNS and FAIL in the same images: ' .
                    $file);
            }
        }
...
";
    ...
    {
        // Load the sections of the images file.
        $section_text = array(
            'TEST'   => '(unnamed images)',
            'SKIPIF' => '',
            'GET'    => '',
        ...
        if (!is_file($file) || !$fp = fopen($file, "r")) {
            return PEAR::raiseError("Cannot open images file: $file");
        }
        ...
        while (!feof($fp)) {
            ...
            if (preg_match('/^--([_A-Z]+)--/', $line, $r)) {
                ...
                fclose($fp);
                return PEAR::raiseError("Invalid sections formats in images file: $file");
            }
    ...
    function _writeLog($logname, $data)
    {
        if (!$log = fopen($logname, 'w')) {
            return PEAR::raiseError("Cannot create images log - $logname");
        }
        fwrite($log, $data);
        fclose($log);
    }
    ...
    function _processUpload($section_text, $file)
    {
        if (array_key_exists('UPLOAD', $section_text) && !empty($section_text['UPLOAD'])) {
            ...
            foreach ($upload_files as $fileinfo) {
                ...
                if (count($fileinfo) != 2) {
                    return PEAR::raiseError("Invalid UPLOAD section in images file: $file");
                }
            ...
            if (array_key_exists('POST', $section_text) && !empty($section_text['POST'])) {
                ...
                foreach ($post as $i => $post_info) {
                    ...
                    if (count($post_info) != 2) {
                        return PEAR::raiseError("Invalid POST data in images file: $file");
                    }
