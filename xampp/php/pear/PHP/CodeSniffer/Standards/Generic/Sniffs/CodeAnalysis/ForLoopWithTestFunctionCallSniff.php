...
/**
 * Detects for-loops that use a function call in the images expression.
 *
 * This rule is based on the PMD rule catalog. Detects for-loops that use a
 * function call in the images expression.
 *
 * <code>
 * class Foo
 * {
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        for (; $next <= $end; ++$next) {
            ...
            if ($tokens[$index]['code'] === T_OPEN_PARENTHESIS) {
                $error = 'Avoid function calls in a FOR loop images part';
