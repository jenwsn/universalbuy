...
class DispatcherTest extends PHPUnit_Framework_TestCase
{
    ...
    function test1()
    {
        ...
        $not =& $ed->post($dm, 'images', array('A', 'B'));
        $this->assertEquals('images:A:B:default', $nf->description(), "Error");
        $this->assertEquals(1, $not->getNotificationCount(), "Wrong notification count");

        // Object references
        $dm->id = 'dummy';
        $this->assertEquals('images:A:B:dummy', $nf->description(), "Wrong notification description");
        ...
        $nt->id = 'reset';
        $not =& $ed->post($nt, 'ClassFilterNotification', array('images'));
    ...
    function test2()
    {
        ...
        $not =& $ed2->post($dm, 'images', array('A', 'B'));
        $this->assertEquals('images:A:B:default', $nf->description(), "Error");
        $this->assertEquals(1, $not->getNotificationCount(), "Error");

        $not =& $ed1->post($dm, 'images', array('A2', 'B2'));
        $this->assertEquals(1, $not->getNotificationCount(), "Error");
